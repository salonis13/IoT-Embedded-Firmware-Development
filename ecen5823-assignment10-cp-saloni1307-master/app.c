...
// Modified from soc_btmesh_switch example from Silicon Labs for
...
SL_WEAK void app_init(void)
{
  ...
  /////////////////////////////////////////////////////////////////////////////
  app_log("BT mesh Client-Publisher initialized\r\n"); // DOS
...
static void set_device_name(bd_addr *addr)
{
  ...
  snprintf(name, NAME_BUF_LEN, "Client-pub %02x:%02x", // DOS
...
bool handle_reset_conditions(void)
{
  ...
  if (sl_simple_button_get_state(sl_button_btn1.context)
      == SL_SIMPLE_BUTTON_PRESSED) {
    // Disable button presses
    app_button_press_disable();
    // Node factory reset
...
void app_button_press_cb(uint8_t button, uint8_t duration)
{
  ...
  // Selecting action by button press duration
  switch (duration) {
...
*/
  ...
  // DOS, 10-06-2021 :
  //
  // We are only supporting button presses between 1s and < 5s.
  // Calling sequence is:
  //   sl_btmesh_change_switch_position()
  //     send_onoff_request()
  //       mesh_lib_generic_client_publish() // send Generic On/Off Request
  //
  // Using the Generic On/Off Client model.
  //
  // See Software Components in the .slcp file:
  //   Bluetooth Mesh / Stack Classes / Generic Client Models, and
  // dcd_config.btmeshconf file:
  //   Main Element
  //
...
#else
      if (button == BUTTON_PRESS_BUTTON_0) {
        sl_btmesh_change_switch_position(SL_BTMESH_LIGHTING_CLIENT_OFF); // PB0 press = Turn Off
      } else {
        sl_btmesh_change_switch_position(SL_BTMESH_LIGHTING_CLIENT_ON);  // PB1 press = Turn On
      }
#endif
      break;
